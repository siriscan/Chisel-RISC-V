error id: file:///C:/Users/irisc/Documents/CHISEL/Chisel-RISC-V/src/main/scala/integer/FetchStage.scala:local0
...
class FetchStage(conf: CoreConfig) extends Module {
  ...
  val btbHit = btb.io.hit
  val predictedPC = btb.io.predictedPC

  // Outputs to Decode Stage
  io.predTaken := btbHit
  io.predTarget := predictedPC

  val Pmem = Module(new InstructionMem(conf.imemSize, conf.imemFile)) // 16KB Instruction Memory from imemFile (pmem.hex)

  // Determine next PC based on branch prediction and actual branch outcome
  val predOrNormal = Mux(btbHit, btb.io.@@predictedPC, pcReg + 4.U) // If BTB hit, use predicted PC; else PC + 4
  nextPc := Mux(io.takeBranch, io.branchTarget, predOrNormal) // If branch taken, go to branchTarget; else predicted PC or PC + 4 (depends on BTB)
