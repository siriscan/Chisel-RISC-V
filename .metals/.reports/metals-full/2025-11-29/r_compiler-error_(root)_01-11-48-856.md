...
import chisel3.util.{is => UInt}
import chisel3.{when => ControlSignals}
import chisel3.{when => UInt}
import integer.{ALUConsts_Interger => Output}
import integer.{ALUConsts_Interger => UInt}
package integer
...
class Decode(conf: @@CoreConfig) extends Module {
  val io = IO(new Bundle {
    // Inputs from Fetch
    val pc   = Input(UInt(conf.xlen.W))
    val inst = Input(UInt(32.W))

    // Inputs from Writeback Stage (to write to RegFile)
    val wbAddr = Input(UInt(5.W))
    val wbData = Input(UInt(conf.xlen.W))
    val wbEn   = Input(Bool())

    // Outputs to Execute Stage
    val ctrl = Output(new ControlSignals)
    val pcOut = Output(UInt(conf.xlen.W))
    val rs1Data = Output(UInt(conf.xlen.W))
    val rs2Data = Output(UInt(conf.xlen.W))
