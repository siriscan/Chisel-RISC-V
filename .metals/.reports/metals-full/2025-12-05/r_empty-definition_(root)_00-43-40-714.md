error id: file:///C:/Users/irisc/Documents/CHISEL/Chisel-RISC-V/src/main/scala/integer/ExecuteStage.scala:integer/ExecuteStage#zeroFlag.
...
non-local guesses:
	 ...
	 -chisel3/zeroFlag#
	 -chisel3/zeroFlag().
	 -chisel3/util/zeroFlag.
	 -chisel3/util/zeroFlag#
	 -chisel3/util/zeroFlag().
	 -zeroFlag.
	 -zeroFlag#
	 -zeroFlag().
	 -scala/Predef.zeroFlag.
	 -scala/Predef.zeroFlag#
	 -scala/Predef.zeroFlag().
offset: 2238
...
class ExecuteStage(config : CoreConfig) extends Module {
  val io = IO(new Bundle {
    ...
    io.branchTaken := (currentBranch && zer@@oFlag) || io.controlSignals.jump // Branch taken if branch signal is high and zero flag is set, or if it's a jump
